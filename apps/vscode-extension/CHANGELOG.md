...
- Adaptive routing now treats empty, malformed or schema-invalid structured review output as a semantic model failure, cools that explicit free model down for subsequent batches, and gives provider daily-limit errors an extended cooldown instead of repeatedly selecting the same unusable model.
