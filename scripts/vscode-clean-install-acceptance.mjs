...
try {
  ...
  await writeFile(
    ...
    [
      ...
      "  }",
      "",
      '  const externalReviewHandlerReady = await vscode.commands.executeCommand("llmatic.reviewExternalPullRequest", { probe: true });',
      '  assert.equal(externalReviewHandlerReady, true, "installed external PR review command handler did not execute");',
      '  const autoReviewHandlerReady = await vscode.commands.executeCommand("llmatic.configureAutoReview", { probe: true });',
      '  assert.equal(autoReviewHandlerReady, true, "installed Auto Review Agent command handler did not execute");',
